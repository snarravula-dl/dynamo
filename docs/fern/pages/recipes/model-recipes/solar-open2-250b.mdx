...
Each Job runs the trace once and writes its summary to `ARTIFACT_DIR`. To repeat a
run, delete the Job and re-apply it; Jobs are immutable, so re-applying without
deleting fails. Results vary run to run by roughly one to two percent, so repeat a
measurement before treating a difference as real.
...
Disaggregated figures are the mean of two runs made with the profile's `perf.yaml`.
That configuration isolates the warmup phase from profiling; it does not clear
server-side KV cache. For runs that must start from a cold cache, restart the worker
pods between them.
